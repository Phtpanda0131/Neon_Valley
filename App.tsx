...
import { INITIAL_STATS, MAX_STAT_POINTS, LIFESTYLES, TRAITS, WEAPON_BASES, CONSUMABLES, BODY_MODS } from './constants.tsx';
...
const App: React.FC = () => {
  ...
  }, [character.stats, character.selectedTraitIds, character.bodyMods]);
  ...
  const handleEndTurn = () => {
    let ncRegen = 0;
    let kpRegen = 0;
    if (character.bodyMods.Core === 'alternator') ncRegen += 3;
    if (character.bodyMods.Core === 'kinetic-battery') kpRegen += 2;

    if (ncRegen > 0) updateVital('chg', 'current', ncRegen);
    if (kpRegen > 0) updateVital('phy', 'current', kpRegen);
  };
  ...
  const getWeaponStats = (weapon: EquippedWeapon) => {
    const base = WEAPON_BASES.find(b => b.id === weapon.baseId);
    if (!base) return { damage: '0', effects: [], hitBonus: 0 };
    ...
    <div className="min-h-screen p-4 md:p-8 relative">
      ...
      <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 relative z-10 animate-fade-in">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <section className="cyber-card p-6 md:p-8 overflow-hidden">
            <h2 className="text-xl font-orbitron font-bold text-white uppercase tracking-widest mb-8 flex items-center gap-3">
              <span className="w-8 h-[1px] bg-pink-900" /> Neural_Matrix <span className="flex-1 h-[1px] bg-pink-900/30" />
            </h2>
            <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 custom-scrollbar transition-all ${isTraitsComplete && !forceExpandTraits ? 'max-h-40' : 'max-h-[600px] overflow-y-auto'}`}>
              {displayTraits.map(t => {
                ...
                return (
                  <button key={t.id} onClick={() => toggleTrait(t.id)} className={`text-left p-4 border transition-all relative overflow-hidden group/trait ${active ? 'bg-pink-600/10 border-pink-500 shadow-[0_0_15px_rgba(236,72,153,0.2)]' : 'bg-slate-900/40 border-slate-800 grayscale hover:grayscale-0 hover:border-slate-600 opacity-60 hover:opacity-100'}`}>
                    <div className="font-orbitron font-black uppercase tracking-tighter text-sm mb-1">{t.name}</div>
                    <div className="text-[9px] text-emerald-400 font-mono uppercase font-black leading-none mb-1">▲ {t.bonus}</div>
                    <div className="text-[9px] text-rose-500 font-mono uppercase font-black leading-none">▼ {t.penalty}</div>
                    {active && <div className="absolute top-0 right-0 w-6 h-6 bg-pink-500 clip-path-cyber flex items-center justify-center"><span className="text-black font-black text-[10px]">✓</span></div>}
                  </button>
                );
              })}
            </div>
          ...
          <section className="cyber-card p-6 md:p-8 bg-tactical border-amber-500/20">
            <div className="flex justify-between items-center mb-10">
              ...
              <div className="flex gap-4">
                 <button onClick={() => setIsHackingOpen(true)} className="px-5 py-2 bg-amber-500/10 border border-amber-500/50 text-amber-500 font-orbitron text-[10px] font-black uppercase hover:bg-amber-500 hover:text-black transition-all">[ HACK_UPLINK ]</button>
              </div>
            </div>
          ...
          <section className="cyber-card p-6 md:p-8">
            ...
            {lastRoll !== null && (
              <div className="mt-6 text-center text-[10px] font-mono text-emerald-500 uppercase animate-pulse">CRITICAL_HEAL_SYNC: {lastRoll} HP RESTORED</div>
            )}
          </section>

          {/* System Integrity footer relocated buttons */}
